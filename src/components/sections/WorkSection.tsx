...
export default function WorkSection({ isStandalonePage = false }: WorkSectionProps) {
  ...
  // ── STEP 1: Pre-hide elements BEFORE first paint (useLayoutEffect = synchronous, before paint) ──
  // This prevents the flash of visible content before GSAP hides it.
  // We set CSS directly (NOT through GSAP context) so GSAP's revert cannot undo this.
  useLayoutEffect(() => {
    if (!isStandalonePage || !wrapperRef.current) return;
    const scope = wrapperRef.current;
    const titleEl = scope.querySelector<HTMLElement>(".mask-title");
    const leftColEl = scope.querySelector<HTMLElement>(".work-left-col");
    const showcaseEl = scope.querySelector<HTMLElement>(".showcase-container");

    // Hide directly via DOM style — invisible from frame 0, immune to GSAP revert
    if (titleEl) { titleEl.style.opacity = "0"; titleEl.style.transform = "translateY(110%)"; }
    if (leftColEl) { leftColEl.style.opacity = "0"; leftColEl.style.transform = "translateY(50px)"; }
    if (showcaseEl) { showcaseEl.style.opacity = "0"; showcaseEl.style.transform = "translateY(50px)"; }
  }, [isStandalonePage]);

  // ── STEP 2: Standalone entrance animation ──
  // Runs as a plain useEffect (not useGSAP) so it has NO GSAP context revert.
  // Fires once when isLoaderFinished becomes true.
  useEffect(() => {
    if (!isStandalonePage || !isLoaderFinished || !wrapperRef.current) return;
    const scope = wrapperRef.current;
    const titleEl = scope.querySelector<HTMLElement>(".mask-title");
    const leftColEl = scope.querySelector<HTMLElement>(".work-left-col");
    const showcaseEl = scope.querySelector<HTMLElement>(".showcase-container");
    ...
    tl.to(titleEl, { y: "0%", opacity: 1, duration: 1.0, ease: "power3.out", clearProps: "transform,opacity" })
      .to(leftColEl, { opacity: 1, y: 0, duration: 1.0, ease: "power3.out", clearProps: "transform,opacity" }, "-=0.6")
      .to(showcaseEl, { opacity: 1, y: 0, duration: 1.0, ease: "power3.out", clearProps: "transform,opacity" }, "-=0.7");

    return () => { tl.kill(); };
  }, [isStandalonePage, isLoaderFinished]);

  // ── STEP 3: Pin + Image Parallax (runs ONCE on mount, no loader dependency) ──
  // Pin must be stable — never reverted/rebuilt mid-session.
  useGSAP(() => {
  ...
  }, { scope: wrapperRef, dependencies: [] }); // Empty deps = stable, runs once only

  // ── STEP 4: Home Page Scroll-Triggered Entrance Animations ──
  // Separate from pin so it can safely depend on isLoaderFinished without breaking the pin.
  useGSAP(() => {
    if (isStandalonePage || !isLoaderFinished || !sectionRef.current || !wrapperRef.current) return;
  ...
  }, { scope: wrapperRef, dependencies: [isStandalonePage, isLoaderFinished] });

  // ── STEP 5: Cinematic Crossfades ──
  ...
  return (
    <div ref={wrapperRef} className={`relative w-full z-30 bg-[#050505]${isStandalonePage ? " -mt-32" : ""}`}>
      ...
      >
        {/* ── Slanted Glowing Top Separator Line (home only) ── */}
        {!isStandalonePage && (
        ...
        <div
          ref={contentRef}
          className={`relative z-10 w-full max-w-[1500px] mx-auto px-6 md:px-16 flex flex-col justify-center gap-6 md:gap-12 h-full will-change-transform ${isStandalonePage ? "pt-32" : ""}`}
        >
          {/* ── LATEST WORK. HEADING ── */}
          <div className="mask-title-wrapper overflow-hidden pb-1 select-none pointer-events-none w-full">
            <h2 className="mask-title text-[clamp(2.5rem,7vw,6.5rem)] font-black uppercase text-[#f04e00] leading-[0.85] tracking-tighter">
              LATEST WORK.
            </h2>
          </div>
